...
export class Memory {
  ...
  getValue(address: string, size?: 'b' | 'w' | 'd', addresses?: string[]) {
    ...
    if (size === 'w') {
      return (
        this.getByteFromAddress(address.toInt() + 1, addresses) + this.getByteFromAddress(address.toInt(), addresses)
      );
    }

    return (
      this.getByteFromAddress(address.toInt() + 3, addresses) +
      this.getByteFromAddress(address.toInt() + 2, addresses) +
      this.getByteFromAddress(address.toInt() + 1, addresses) +
      this.getByteFromAddress(address.toInt(), addresses)
    );
  }
