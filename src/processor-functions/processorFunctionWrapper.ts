...
export const processorFunctionWrapper = (fn: (f: IFunction) => any, instruction: Instruction, processor: Processor, options?: { defaultBehavior?: boolean, flags?: flagType[], incrementPointer?: boolean }) => {
...
const processMemory = (op: operand, processor: Processor, pointer: 'b' | 'w' | 'd'): { address: string, value: string } => {
    let registerValue = 0;
    ...
    return {value: processor.getMemory().getValue(value).substr(8 - pLength), address: value};
};
...
const processFlags = (processor: Processor, flags: flagType[], op: operand, value1: string, value2: string, result: string) => {
